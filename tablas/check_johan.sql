...
alter table habitacion add constraint check_tipo_habitacion_habitacion check (tipo_habitacion > 0);
...
alter table cliente add constraint check_fecha_nacimiento_cliente check (fecha < current_date);
...
alter table cliente add constraint check_pais_cliente check (pais > 0);
...
alter table detalle_comprobante add constraint check_monto_total_sevicio_detalle_comprobante check (monto_total_sevicio > 0.00 :: money);
...
alter table tipo transaccion add constraint check_descripcion_tipo_transaccion check (~* '^[[a-z\sá-úÁ-Ú]{1,15}$');
